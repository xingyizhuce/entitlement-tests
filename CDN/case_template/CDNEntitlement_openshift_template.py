...
from CDN import openshift_version
from CDN import openshift_repo
...
class CDNEntitlement_PID(unittest.TestCase):
    def setUp(self):
        ...
        try:
            ...
            if self.pid in ["290", "482"]:
                self.base_pid = CDN().get_default_pid_cert(self.system_info)
            else:
            ...
            # Openshift_addon_ids,for this ids need enable some special repo so list here to handle specially.
            openshift_addon_ids = ["290","482"]

            if self.pid in openshift_addon_ids:
                self.openshift_repo = openshift_repo[openshift_version][self.base_pid]

            rhel_8_addon_ids = ["487", "488", "230", "489", "233",
                                "232"]  # These ids are different with others, different arches have
            ...
            CDNParseManifestXML(self.manifest_url, self.manifest_path, self.manifest_json,
                                self.manifest_xml).parse_json_to_xml()
    ...
    def testCDNEntitlement_VARIANT_ARCH_PID(self):
        ...
        try:
            ...
            test_result &= CDN().check_default_pid_cert(self.system_info, self.manifest_xml, self.default_pid,
                                                        self.current_arch, self.platform_version)
            ...
            if self.base_sku in sku_pool_dict.keys():
                ...
                self.assertTrue(result,
                                msg="Test Failed - Failed to subscribe with base sku {0}!".format(self.base_sku))
                ...
                if ent_cert != "":
                    ...
                    test_result = False
                    logger.error(
                        "Test Failed - Failed to get entitlement certificate after subscribe base sku {0}.".format(
                            self.base_sku))
            else:
            ...
            if self.base_sku != self.sku:
                if self.sku in sku_pool_dict.keys():
                    ...
                    if ent_cert != "":
                        ...
                        result &= CDN().verify_arch_in_ent_cert(self.system_info, ent_cert, self.manifest_xml, self.pid,
                                                                self.current_release_ver)
                    else:
                        test_result = False
                        logger.error(
                            "Test Failed - Failed to get entitlement certificate after subscribe layered sku {0}.".format(
                                self.sku))
                else:
            ...
            if test_type == "GA":
                ...
                if self.release_ver != self.current_release_ver:
                    releasever_set = CDN().set_release(self.system_info, self.release_ver, self.manifest_json,
                                                       master_release)
            ...
            if not test_enabled_repo:
                ...
                if master_release == "8":
                    ...
                    self.assertTrue(result,
                                    msg="Test Failed - Failed to enable default repo: {0}".format(default_repo_1))

                    result = CDN().enable_repo(self.system_info, default_repo_2)
                    self.assertTrue(result,
                                    msg="Test Failed - Failed to enable default repo: {0}".format(default_repo_2))
                else:
            ...
            # If 0 repo, exit
            repo_list = CDN().get_repo_list_from_manifest(self.manifest_xml, self.pid, self.current_arch,
                                                          self.release_ver)
            if self.pid in ["290", "482"] and CDN().get_default_pid_cert(self.system_info) == "279":
                repo_list = [i for i in repo_list if "power-le" in i]
                print repo_list
            elif self.pid in ["290", "482"] and CDN().get_default_pid_cert(self.system_info) == "420":
                repo_list = [i for i in repo_list if "power-9" in i]
                print repo_list

            self.assertNotEqual(repo_list, [],
                                msg="Test Failed - There is no any repository for pid {0} in release {1}.".format(
                                    self.pid, self.release_ver))

            # Make sure all test repos listed in manifest can be retrieved successfully
            test_result &= CDN().test_repos(self.system_info, repo_list, releasever_set, self.release_ver,
                                            self.current_arch)
            ...
            if master_release == "7":
                if test_type == "Beta":
                    ...
                    if self.pid in ["290","482"]:
                        result = CDN().enable_ocprepo(self.system_info,self.openshift_repo)
                        self.assertTrue(result, msg="Test Failed - Failed to enable openshift repo!")
            ...
            for repo in repo_list:
                ...
                if self.pid in ["290", "482"]:
                    test_result &= CDN().package_full_installation(self.system_info, repo, releasever_set,
                                                                   self.manifest_xml, self.pid, self.base_pid,
                                                                   self.default_pid, self.current_arch,
                                                                   self.release_ver,
                                                                   self.platform_version, self.openshift_version, master_release)
                else:
                    test_result &= CDN().package_smoke_installation(self.system_info, repo, releasever_set,
                                                                self.manifest_xml, self.pid, self.base_pid,
                                                                self.default_pid, self.current_arch, self.release_ver,
                                                                self.platform_version, master_release)
